...
3. Select **Original**, **Freeform**, or a common aspect ratio. In **Freeform**, drag any edge to
   resize the crop. You can also focus an edge and use the Arrow keys. Hold Shift for larger steps.
4. Drag the image or use the Arrow keys to position it. Adjust **Zoom** until the crop is smaller
   than the original image.
5. Select one of the crop actions:
   - **Duplicate and crop** creates a separate media item and leaves the original unchanged. Select
     the cropped copy in each content entry where you want to use it.
   - **Crop original** replaces the image everywhere the media item is used. Existing content
     entries keep the same media reference and are not rewritten or republished.

<Aside type="caution">
	**Crop original** cannot be undone. EmDash does not keep the uncropped bytes or a crop history.
	It is available only with the **Original** aspect ratio. Use **Duplicate and crop** for Freeform
