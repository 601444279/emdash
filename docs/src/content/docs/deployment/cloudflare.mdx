...
On Cloudflare Workers, scheduled publishing, plugin tasks, and background media indexing run from Cron Triggers. New Cloudflare templates configure both triggers automatically. For an existing project, configure one trigger for general scheduled tasks and another for media indexing:
...
<Aside type="caution">
	Without the general trigger, scheduled publishing and plugin tasks do not run. Without
	`mediaUsageCron`, existing content does not catch up in the background. Local `astro dev` uses
	the built-in scheduler instead.
</Aside>
...
Also do not confuse either of those with **object cache** (`objectCache: kvCache({ binding: "CACHE" })`), which caches database query results in KV as a separate layer under the Worker.
...
email-dependent flows such as magic-link login, team invites, and comment
...
| `replyTo` | `string`                     | None         | Optional Reply-To, useful when `from` is a no-reply subdomain address. |
