...
The Media Usage Cron sends a recovery wake to this Queue. Each consumer invocation processes as many
bounded, durable database units as fit safely within one Workers Paid event, then sends another wake
while work remains. Queue messages contain no content identity, and the existing database work rows
remain authoritative if a message is retried or lost. Concurrency stays at one because D1 processes a
database sequentially.

Existing custom Workers without a Queue binding run a bounded maintenance slice on each Media Usage
Cron event. To use different schedules, set the corresponding `generalCron` or `mediaUsageCron`
option in `createScheduledHandler()` and use the same expression in `wrangler.jsonc`.

<Aside type="caution">
	Without the general trigger, scheduled publishing and plugin cron do not run. Without the dedicated Media Usage trigger, an interrupted or exhausted Queue chain cannot recover automatically. Local `astro dev` still uses the in-process scheduler.
</Aside>
...
you close the page. The Media Usage Cron recovers setup or indexing after an interrupted Queue chain.
