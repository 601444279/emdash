...
The general schedule also sends Media Usage recovery wakes. Add one Cron Trigger, the five-minute CPU allowance, and the automatically provisioned Queue to `wrangler.jsonc`:
...
The general Cron sends a Media Usage recovery wake to this Queue. Each batch processes up to 1,000
entries. The consumer continues with another batch while its invocation has capacity, then sends
another wake when work remains. Queue messages contain no content identity, and the existing database
work rows remain authoritative if a message is retried or lost. Concurrency stays at one because D1
processes a database sequentially.

Existing custom Workers without a Queue binding run a bounded maintenance slice during each general
Cron event. To use a different schedule, set `generalCron` in `createScheduledHandler()` and use the
same expression in `wrangler.jsonc`. The `mediaUsageCron` option remains available for deployments that
already use a separate compatibility trigger.

<Aside type="caution">
	Without the general trigger, scheduled publishing, plugin cron, and interrupted Media Usage Queue chains cannot recover automatically. Local `astro dev` still uses the in-process scheduler.
</Aside>

### Enable automatic media usage indexing

Keep general scheduled maintenance running, then pause all application and direct database writes. Open
...
you close the page. The general Cron recovers setup or indexing after an interrupted Queue chain.
