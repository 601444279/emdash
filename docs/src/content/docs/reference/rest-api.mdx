...
in a partial list can carry a term past siblings that list didn't include. For
example, `[A, B, C]` is the full group, but `B` is not translated into the
locale you are working in. If you send `["C", "A"]`, the result is `[C, B, A]`.
`A` and `C` swap as requested, and a listing that includes `B` shows `A` moving
